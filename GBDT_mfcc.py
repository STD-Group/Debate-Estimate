...
#         learning_curve(gbc, xscaled, y, train_sizes=np.linspace(0.2, 1.0, numSection),\
...
np.save("TestAns/GBDT_MFCC.npy", yTestGBDT)
...
np.save("TestAns/GBDT_LR_MFCC.npy", yTestLR)
...
#         xNewTest = np.array(encoder.transform(tmpTest).toarray())
#
#         lrcv.fit(tmpTrain, y_train)
#         coef = lrcv.coef_
#
#         yPredict = lrcv.predict(tmpTrain)
#         acc = (yPredict == y_train).sum() / y_train.size
#         # print('LR train acc is {:.4f}'.format(acc))  # 精度
#         lrTrainAcc += acc
#
#         yPredict = lrcv.predict(tmpTest)
#         acc = (yPredict == y_test).sum() / y_test.size
